...
void LivroRetira( ListaLivro* biblio, ListaAluno* alunos ) {
	...
	if (esteL->Qnt == 0 ) {
		...
		printf("\n");
		printf("  -> O livro %s esta cadastrado na biblioteca e %d copias do mesmo estao disponiveis para locacao. Pressione ENTER para continuar.", esteL->Titulo, esteL->Qnt);
		EsperaEnter();
	}
	
	// Busca o aluno que o locara
	Aluno* esteA = AlunoBusca( alunos );
	if (esteA == NULL) {
		printf("  -> Falha na locacao de livro. Pressione ENTER para continuar.");
		EsperaEnter();
		return;
	...
	if (addFila) {
		...
		sprintf( mensagem, "Voce agora esta na fila de espera para a locacao do livro %s, na posicao %d. Voce sera notificado quando o livro estiver disponivel para voce o alugar.", esteL->Titulo, FilaEsperaTamanho(&esteL->Espera) );
		PilhaMsgPush(&esteA->Mensagens, mensagem );
		
		// Fim da rotina
		printf("  -> O aluno %s agora esta no final da fila de espera para o livro %s. Pressione ENTER para continuar.", esteA->Nome, esteL->Titulo );
		EsperaEnter();
	}
	else {		// Caso a locacao ocorra, sem necessidade de utilizar a fila de espera
		printf("  -> O livro %s sera locado por %s. Prosseguir? (s/n): ", esteL->Titulo, esteA->Nome );
		LimpaBuffer();
		char opcao2;
		opcao2 = getchar();
		printf("\n");
		...
		if ( (opcao2 == 's') || (opcao2 == 'S') ) {
			...
			/* Uma mensagem é alocada, com o tamanho exato (com uma pequena margem) necessario
		     (tamanho da mensagem = mensagem + titulo do livro) */
			string mensagem = (char *)malloc( ( strlen(esteL->Titulo) + 23 ) * sizeof(char) );
			if (mensagem == NULL) {
				printf("ERRO: o sistema encontrou um problema! (Falha em alocação de memoria).\n");
				abort();
			}
			...
			sprintf( mensagem, "Voce alugou o livro %s.", esteL->Titulo );
			PilhaMsgPush( &esteA->Mensagens, mensagem );
...
void LivroRetorna( ListaLivro* biblio, ListaAluno* alunos ) {
	...
	if ( (opcao == 's') || (opcao == 'S') ) {
		...
		/* Uma mensagem é alocada, com o tamanho exato (com uma pequena margem) necessario
		 (tamanho da mensagem = mensagem + titulo do livro) */
		string mensagem = (char *)malloc( ( sizeof(esteL->Titulo) + 25 ) * sizeof(char) );
		if (mensagem == NULL) {
			printf("ERRO: o sistema encontrou um problema! (Falha em alocação de memoria).\n");
			abort();
		}
		...
		sprintf( mensagem, "Voce devolveu o livro %s.", esteL->Titulo );
		PilhaMsgPush(&esteA->Mensagens, mensagem );
		...
		Aluno* ProximoFila = FilaEsperaAnda( &esteL->Espera );
		if (ProximoFila != NULL) {

			/* Uma mensagem é alocada, com o tamanho exato (com uma pequena margem) necessario
			 (tamanho da mensagem = mensagem + titulo do livro) */
			mensagem = (char*)realloc( mensagem, ( sizeof(esteL->Titulo) + 73 ) * sizeof(char) );
			if (mensagem == NULL) {
				printf("ERRO: o sistema encontrou um problema! (Falha em alocação de memoria).\n");
				abort();
			}
			...
			// Envia a mensagem ao aluno
			sprintf( mensagem, "O livro %s agora esta disponivel para voce o locar. Venha o quanto antes.", esteL->Titulo );
...
void AlunoCadastra( ListaAluno* alunos ) {
	...
	string mensagem = (char *)malloc( (strlen(temp->Nome) + 40 ) * sizeof(char) );
	if (mensagem == NULL) {
		printf("ERRO: o sistema encontrou um problema! (Falha em alocação de memoria).\n");
		abort();
	}
...
void MensagensNaoLidas( ListaAluno * alunos ) {
	...
	// Busca um aluno
	Aluno* este = AlunoBusca( alunos );
	if (este != NULL) // Caso encontre, imprime suas mensagens nao lidas
		PrintaMensagens ( este );
